...
var ErrCodeName = map[ErrCode][]string{
	...
	ERR_CODE_CHECK_PAYLOAD_PARAM_SUBSCRIBE_TX:             {"check subscribe tx payload failed", "校验订阅交易Payload数据失败"},
	ERR_CODE_CHECK_PAYLOAD_PARAM_SUBSCRIBE_CONTRACT_EVENT: {"check subscribe contract event payload failed", "校验订阅合约事件Payload数据失败"},

...
func NewEasyCodecWithMap(value map[string]string) *EasyCodec {
...
func (e *EasyCodec) AddMap(value map[string]string) {
	items := ParamsMapToEasyCodecItem(value)
...
func (e *EasyCodec) ToMap() map[string]string {
...
func ParamsMapToEasyCodecItem(params map[string]string) []*EasyCodecItem {
	...
	for _, key := range keys {
		items = append(items, newEasyCodecItemWithString(key, params[key]))
	}
	return items
}
...
func EasyCodecItemToParamsMap(items []*EasyCodecItem) map[string]string {
	...
	for _, item := range items {
		...
		case EasyValueType_BYTES:
			params[item.Key] = string(item.Value.([]byte))
			break
		case EasyValueType_INT32:
			params[item.Key] = strconv.Itoa(int(item.Value.(int32)))
			break

...
func addGMTrust(pool *sm2.CertPool, path string) error {
	aCrt, err := ioutil.ReadFile(path)
	if err != nil {
		return fmt.Errorf("read cert file failed, %s", err.Error())
	}
...
func getSM2Certificates(trustRoot string) ([]*sm2.Certificate, error) {
	var certificates []*sm2.Certificate
...
func addSM2CertPool(certPool *sm2.CertPool, trustRoot string) error {

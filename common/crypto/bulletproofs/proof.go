...
type bulletproofsImpl struct{}

type provider struct{}

func (*provider) NewBulletproofs() bulletproofs {
	return new(bulletproofsImpl)
}
...
func (b *bulletproofsImpl) ProveRandomOpening(x uint64) ([]byte, []byte, []byte, error) {
...
func (b *bulletproofsImpl) ProveSpecificOpening(x uint64, opening []byte) ([]byte, []byte, error) {
...
func (b *bulletproofsImpl) Verify(proof []byte, commitment []byte) (bool, error) {
...
func (b *bulletproofsImpl) ProveAfterAddNum(x, y uint64, openingX, commitmentX []byte) ([]byte, []byte, error) {
	ret, err := b.PedersenVerify(commitmentX, openingX, x)
	if err != nil {
	...
	proof, commitment, err := b.ProveSpecificOpening(z, openingX)
	if err != nil {
		return nil, nil, err
	}
	commitmentDup, err := b.PedersenAddNum(commitmentX, y)
	if err != nil {
...
func (b *bulletproofsImpl) ProveAfterAddCommitment(x, y uint64, openingX, openingY, commitmentX, commitmentY []byte) ([]byte, []byte, []byte, error) {
	ret, err := b.PedersenVerify(commitmentX, openingX, x)
	if err != nil {
	...
	ret, err = b.PedersenVerify(commitmentY, openingY, y)
	if err != nil {
		return nil, nil, nil, fmt.Errorf(ERR_MSG_INVALID_INPUT + ": " + err.Error())
	}
	...
	commitmentDup, opening, err := b.PedersenAddCommitmentWithOpening(commitmentX, commitmentY, openingX, openingY)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("fail to generate proof: " + err.Error())
	}

	proof, commitment, err := b.ProveSpecificOpening(z, opening)
	if err != nil {
...
// return 1: proof in []byte
// return 2: commitment of x - y: (x - y)B + rB'
func (b *bulletproofsImpl) ProveAfterSubNum(x, y uint64, openingX, commitmentX []byte) ([]byte, []byte, error) {
	ret, err := b.PedersenVerify(commitmentX, openingX, x)
	if err != nil {
		return nil, nil, fmt.Errorf(ERR_MSG_INVALID_INPUT + ": " + err.Error())
	}
	...
	commitmentDup, err := b.PedersenSubNum(commitmentX, y)
	if err != nil {
		return nil, nil, fmt.Errorf("fail to generate proof: " + err.Error())
	}
	if bytes.Compare(commitment, commitmentDup) != 0 {
...
// return 2: commitment of x - y: (x - y)B + (r - s)B'
// return 3: new opening for the result commitment (r - s)
func (b *bulletproofsImpl) ProveAfterSubCommitment(x, y uint64, openingX, openingY, commitmentX, commitmentY []byte) ([]byte, []byte, []byte, error) {
	ret, err := b.PedersenVerify(commitmentX, openingX, x)
	if err != nil {
		return nil, nil, nil, fmt.Errorf(ERR_MSG_INVALID_INPUT + ": " + err.Error())
	}
	...
	commitmentDup, opening, err := b.PedersenSubCommitmentWithOpening(commitmentX, commitmentY, openingX, openingY)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("fail to generate proof: " + err.Error())
	}
...
func (b *bulletproofsImpl) ProveAfterMulNum(x, y uint64, openingX, commitmentX []byte) ([]byte, []byte, []byte, error) {
	ret, err := b.PedersenVerify(commitmentX, openingX, x)
	if err != nil {
		return nil, nil, nil, fmt.Errorf(ERR_MSG_INVALID_INPUT + ": " + err.Error())
	}
	...
	proof, commitment, err := b.ProveSpecificOpening(z, openingSlice)
	if err != nil {
		return nil, nil, nil, err
	}
	commitmentDup, openingDup, err := b.PedersenMulNumWithOpening(commitmentX, openingX, y)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("fail to generate proof: " + err.Error())
	}
